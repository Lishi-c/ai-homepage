...
| Endpoint | `POST https://app-dfz0c2wu0jcx-api-BYdwQyx58E6L-gateway.appmiaoda.com/caipiao/class` |
| Auth | `platform_managed`，`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| Third-party Domain | `app-dfz0c2wu0jcx-api-BYdwQyx58E6L-gateway.appmiaoda.com` |
...
async function queryCaipiaoClass(): Promise<CaipiaoClassItem[]> {
  const response = await fetch(
    "https://app-dfz0c2wu0jcx-api-BYdwQyx58E6L-gateway.appmiaoda.com/caipiao/class",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- **调用注意**：分类数据变化频率低，建议前端缓存结果，避免重复调用。
